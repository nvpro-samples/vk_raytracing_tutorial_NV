...
# NVIDIA Vulkan Ray Tracing Tutorial - Intersection Shader

![intersection](images/ray_tracing_intersection.png)

This tutorial chapter shows how to use intersection shader and render different primitives with different materials.

Note: this is an extension to the [ray tracing tutorial](../ray_tracing__simple)
...
* Create `VkGeometryNV` from `VkGeometryAABBNV`
...
Then we need the Aabb structure holding all the spheres, but also used for the creation of the BLAS (`VK_GEOMETRY_TYPE_AABBS_NV`). 
...
What is changing compare to triangle primitive is the Aabb data (see Aabb structure) and the geometry type (`VK_GEOMETRY_TYPE_AABBS_NV`).
...
The intersection shader is added to the Hit Group `VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_NV`. In our example, we already have a Hit Group for triangle and a closest hit associated. We will add a new one, which will become the Hit Group ID (1), see the TLAS section.
